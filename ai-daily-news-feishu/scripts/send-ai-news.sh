...
# 从 /home/workspace/.env 读取 FEISHU_WEBHOOK
ENV_FILE="/home/workspace/.env"
if [ -f "${ENV_FILE}" ]; then
    FEISHU_WEBHOOK=$(grep -E "^FEISHU_WEBHOOK=" "${ENV_FILE}" | cut -d'=' -f2-)
fi

if [ -z "${FEISHU_WEBHOOK}" ]; then
    echo "ERROR_NO_WEBHOOK: 未找到 FEISHU_WEBHOOK 配置" >&2
    echo "请向用户获取飞书 Webhook 地址，并写入 /home/workspace/.env 文件" >&2
    exit 3
fi
